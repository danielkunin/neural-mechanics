...
for lr in 0.1; do
  for wd in 0.0 0.0001; do
    for beta in 0.8 0.9 0.99; do
      for gmama in 0 0.01 0.01; do
        python extract-momentum.py --save-dir=/mnt/fs6/jvrsgsty/neural-mechanics --experiment=vgg16-bn --expid=timnet_momentum_lr_${lr}_wd_${wd}_beta_${beta}_gamma_${gamma} --overwrite --gpu=0
      done
    done
  done
done
